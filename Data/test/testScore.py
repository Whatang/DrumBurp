...
from Data.DBErrors import BadTimeError, OverSizeMeasure, BadNoteError
...
class TestNoteControl(unittest.TestCase):
    ...
    def testGetNote(self):
        self.assertEqual(self.score.getNote(0, 0, 0, 0), EMPTY_NOTE)

    def testGetNote_BadTime(self):
        self.assertRaises(BadTimeError, self.score.getNote, -1, 0, 0, 0)
        self.assertRaises(BadTimeError, self.score.getNote, 20, 0, 0, 0)
        self.assertRaises(BadTimeError, self.score.getNote, 0, -1, 0, 0)
        self.assertRaises(BadTimeError, self.score.getNote, 0, 20, 0, 0)
        self.assertRaises(BadTimeError, self.score.getNote, 0, 0, -1, 0)
        self.assertRaises(BadTimeError, self.score.getNote, 0, 0, 20, 0)

    def testGetNote_BadNote(self):
        self.assertRaises(BadNoteError, self.score.getNote, 0, 0, 0, -1)
        self.assertRaises(BadNoteError, self.score.getNote, 0, 0, 0,
                          len(self.score.drumKit))

    def testAddNote(self):
        self.score.addNote(0, 0, 0, 0, "o")
        self.assertEqual(self.score.getNote(0, 0, 0, 0), "o")

    def testAddNote_BadTime(self):
        self.assertRaises(BadTimeError, self.score.addNote, -1, 0, 0, 0, "x")
        self.assertRaises(BadTimeError, self.score.addNote, 20, 0, 0, 0, "x")
        self.assertRaises(BadTimeError, self.score.addNote, 0, -1, 0, 0, "x")
        self.assertRaises(BadTimeError, self.score.addNote, 0, 20, 0, 0, "x")
        self.assertRaises(BadTimeError, self.score.addNote, 0, 0, -1, 0, "x")
        self.assertRaises(BadTimeError, self.score.addNote, 0, 0, 20, 0, "x")

    def testAddNote_BadNote(self):
        self.assertRaises(BadNoteError, self.score.addNote, 0, 0, 0, -1, "x")
        self.assertRaises(BadNoteError, self.score.addNote, 0, 0, 0,
                          len(self.score.drumKit), "x")

    def testAddNote_DefaultHead(self):
        self.score.addNote(0, 0, 0, 0)
        defaultHead = self.score.drumKit[0].head
        self.assertEqual(self.score.getNote(0, 0, 0, 0), defaultHead)

    def testDeleteNote(self):
        self.score.addNote(0, 0, 0, 0, "o")
        self.score.deleteNote(0, 0, 0, 0)
        self.assertEqual(self.score.getNote(0, 0, 0, 0), EMPTY_NOTE)

    def testDeleteNote_BadTime(self):
        self.assertRaises(BadTimeError, self.score.deleteNote, -1, 0, 0, 0)
        self.assertRaises(BadTimeError, self.score.deleteNote, 20, 0, 0, 0)
        self.assertRaises(BadTimeError, self.score.deleteNote, 0, -1, 0, 0)
        self.assertRaises(BadTimeError, self.score.deleteNote, 0, 20, 0, 0)
        self.assertRaises(BadTimeError, self.score.deleteNote, 0, 0, -1, 0)
        self.assertRaises(BadTimeError, self.score.deleteNote, 0, 0, 20, 0)

    def testDeleteNote_BadNote(self):
        self.assertRaises(BadNoteError, self.score.deleteNote, 0, 0, 0, -1)
        self.assertRaises(BadNoteError, self.score.deleteNote, 0, 0, 0,
                          len(self.score.drumKit))

    def testToggleNote(self):
        self.score.toggleNote(0, 0, 0, 0, "o")
        self.assertEqual(self.score.getNote(0, 0, 0, 0), "o")
        self.score.toggleNote(0, 0, 0, 0, "o")
        self.assertEqual(self.score.getNote(0, 0, 0, 0), EMPTY_NOTE)

    def testToggleNote_BadTime(self):
        self.assertRaises(BadTimeError, self.score.toggleNote, -1, 0, 0, 0, "x")
        self.assertRaises(BadTimeError, self.score.toggleNote, 20, 0, 0, 0, "x")
        self.assertRaises(BadTimeError, self.score.toggleNote, 0, -1, 0, 0, "x")
        self.assertRaises(BadTimeError, self.score.toggleNote, 0, 20, 0, 0, "x")
        self.assertRaises(BadTimeError, self.score.toggleNote, 0, 0, -1, 0, "x")
        self.assertRaises(BadTimeError, self.score.toggleNote, 0, 0, 20, 0, "x")

    def testToggleNote_BadNote(self):
        self.assertRaises(BadNoteError, self.score.toggleNote, 0, 0, 0, -1, "x")
        self.assertRaises(BadNoteError, self.score.toggleNote, 0, 0, 0,
                          len(self.score.drumKit), "x")

    def testToggleNote_DefaultHead(self):
        self.score.toggleNote(0, 0, 0, 0)
        defaultHead = self.score.drumKit[0].head
        self.assertEqual(self.score.getNote(0, 0, 0, 0), defaultHead)
        self.score.toggleNote(0, 0, 0, 0)
        self.assertEqual(self.score.getNote(0, 0, 0, 0), EMPTY_NOTE)
